...
Examples:
  ...
  hyp host hyper://1234..af/
  hyp host hyper://1234..af/ hyper://fedc..21/
`

export default {
  name: 'host',
  description: 'Sync and host hyper data.',
  usage: {
    simple: '{urls...}',
    full: FULL_USAGE
  },
  command: async function (args) {
    ...
    var statusLines = keys.map(k => `${chalk.bold(short(k))}: Accessing network...`)
    ...
    keys.forEach(async (key, i) => {
      ...
      // periodically trigger a download of the current state
      const triggerDownload = async () => {
        if (struct.type === 'hyperdrive') {
          await struct.api.promises.download('/').catch(e => undefined)
        } else if (struct.type === 'hyperbee') {
          await struct.api.feed.download().catch(e => undefined)
        } else {
          await struct.api.download().catch(e => undefined)
        }
        setTimeout(triggerDownload, 10e3).unref()
      }
      triggerDownload()
    })
  }
