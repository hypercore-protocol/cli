...
Options:
  ...
  --allow-origin {string} - Set Access-Control-Allow-Origin header.
                            Implicitly enables other CORS headers.
...
export default {
  ...
  command: async function (args) {
    ...
    const server = http.createServer(createRequestHandler(drive, args))
    server.listen(port)
...
function createRequestHandler (drive, args) {
  return async function (req, res) {
    var headers = {}
    const allowOrigin = args['allow-origin'] ?? args['allowOrigin']
    ...
    const respondRedirect = (url) => {
      res.writeHead(200, Object.assign(headers, {'Content-Type': 'text/html'}))
