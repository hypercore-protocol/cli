...
class HyperStructure extends EventEmitter {
  ...
  async load () {
    ...
    if (!this.type || !this.header) {
      ...
      await core.ready()
      getClient().network.configure(core, {lookup: true, announce: true})
  ...
  async create (type) {
    ...
    getClient().network.configure(this.core, {lookup: true, announce: true})
  }
...
export async function get (key, {expect} = {expect: undefined}) {
